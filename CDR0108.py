...
def compute_kgram_uniformity_loss(sampled_binary, k=8, samples_per_seq=2048):
    """
    sampled_binary: [B, L] in {0,1} (可以是 straight-through 输出)
    目标：让随机抽取的 k-bit 模式接近均匀分布（减少可压缩结构，帮助 Universal）
    """
    B, L = sampled_binary.shape
    if L < k + 1:
        return torch.tensor(0.0, device=sampled_binary.device, dtype=sampled_binary.dtype)

    # 随机采样起点
    max_start = L - k
    m = min(samples_per_seq, max_start)
    idx = torch.randint(0, max_start, (B, m), device=sampled_binary.device)

    # 取出窗口 [B, m, k]
    ar = torch.arange(k, device=sampled_binary.device).view(1, 1, k)
    bits = sampled_binary.gather(1, (idx.unsqueeze(-1) + ar).view(B, -1)).view(B, m, k)

    # 转成 0..(2^k-1)
    weights = (2 ** torch.arange(k - 1, -1, -1, device=sampled_binary.device)).view(1, 1, k).to(bits.dtype)
    symbols = (bits * weights).sum(dim=2).long()  # [B, m]

    bins = 2 ** k
    hist = torch.zeros((B, bins), device=sampled_binary.device, dtype=torch.float32)
    ones = torch.ones_like(symbols, dtype=torch.float32)
    hist.scatter_add_(1, symbols, ones)

    p = (hist + 1e-3) / (hist.sum(dim=1, keepdim=True) + 1e-3 * bins)
    u = torch.full_like(p, 1.0 / bins)

    # KL(p||u) 越小越好
    kl = (p * (torch.log(p + 1e-12) - torch.log(u))).sum(dim=1).mean()
    return kl.to(sampled_binary.dtype)
...
def compute_loss(output):
    ...
    # ✅ 新增：k-gram 均匀性（建议先用 k=8；如果还差一点，改 k=10）
    loss_kgram = compute_kgram_uniformity_loss(sampled_binary, k=8, samples_per_seq=2048)
    ...
    total_loss = (
        ...
        0.6 * loss_flat +
        0.4 * loss_autocorr +
        0.8 * loss_kgram      # ✅ 新增权重：先用 0.8，过强可降到 0.4
    )

    items = {
        'total_loss': total_loss, 'entropy': loss_entropy, 'balance': loss_balance,
        'trans': loss_trans, 'autocorr': loss_autocorr, 'lrun': loss_lrun, 'flat': loss_flat,
        'cumsum': loss_cumsum, 'kgram': loss_kgram
    }
    ...
    return total_loss, items['entropy'], items['balance'], items['trans'], items['autocorr'], items['lrun'], items['flat'], items['cumsum'], items['kgram']
...
@torch.no_grad()
def save_generated_sequences(model, data_loader, device, output_path="train_CNN2.txt", sample_with_bernoulli=True):
    model.eval()
    all_sequences = []
...
def train(model, train_loader, val_loader, optimizer, scheduler, device, epochs=120, patience=12, use_amp=True):
    ...
    for epoch in range(epochs):
        model.train()
        totals = {name: 0.0 for name in ['loss', 'entropy', 'balance', 'trans', 'autocorr', 'lrun', 'flat', 'cumsum', 'kgram']}
        batch_times = []
        ...
        for batch_idx, bitstream in enumerate(train_loader):
            ...
            with torch.amp.autocast(device_type='cuda' if device.type == 'cuda' else 'cpu', enabled=use_amp):
                ...
                (loss,
                 ...
                 loss_flat,
                 loss_cumsum,
                 loss_kgram) = compute_loss(output)
            ...
            totals['kgram'] += (loss_kgram.item() if not (torch.isnan(loss_kgram) or torch.isinf(loss_kgram)) else 0.0)
        ...
        print(
            ...
            f"K-gram: {totals['kgram']/max(1,len(train_loader)):.6f} | "
...
if __name__ == "__main__":
    ...
    save_generated_sequences(model, train_loader, device, output_path="train_CNN1.txt", sample_with_bernoulli=True)

    torch.save(model.state_dict(), "debiased_cnn1.pth")
    print("训练完成！模型已保存到 debiased_cnn1.pth")
